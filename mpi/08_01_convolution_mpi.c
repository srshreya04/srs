...
int main ( int argc, char **argv ) {
  ...
  if (argc >= 3) {
    DIM = atoi(argv[1]);
    GRID_WIDTH = DIM * DIM;
    KERNEL_DIM = atoi(argv[2]); // argument to integer - atoi
  ...
  memset(main_grid, 0, GRID_WIDTH * sizeof(int)); // to initialize arrays to zero
...
//   }
  ...
  for ( iters = 0; iters < num_iterations; iters++ ) {
    ...
    memcpy(lower, &main_grid[DIM * (end - num_pads + 1)], sizeof(int) * DIM * num_pads); // memcpy - to copy multiple elements at once, instead of using a loop to copy each element individually
   ...
   // Output the updated grid state
     if ( ID == 0 ) { 
      ...
      //   printf( "\n" ); 
        ...
        FILE *fp = fopen("exec_times.txt", "a"); // append mode
    if(fp) {
        fprintf(fp, "%d %f\n", num_procs, exec_time);
        fclose(fp);
    }
  ...
  if (ID == 0) {
    // Gnuplot commands
    system("gnuplot -e \"set terminal png size 800,600; "
           "set output 'exec_time_vs_procs.png'; "
           "set title 'Execution Time vs Number of Processes'; "
           "set xlabel 'Number of Processes'; "
           "set ylabel 'Execution Time (s)'; "
           "set grid; "
           "plot 'exec_times.txt' using 1:2 with linespoints lw 2 lc rgb 'blue' title 'Execution Time'\"");
}
